...
"use strict";var ContextExtractor=require("./context-extractor");module.exports=function($container){var template=require("./context-show.html");var $html=$(template);if(!window.Flow.channel.run.getCurrentConfig)return console.warn("You're using an older version of FlowJS which does not support viewing model context."),!1;var config=window.Flow.channel.run.getCurrentConfig();var modelType=config.model.split(".")[1];if("vmf"===modelType)return console.info("Cannot view context for Vensim models."),!1;var file=new F.service.File(config);file.getFileContents(config.model,"model").then(function(response){var extractor=new ContextExtractor(config.model,response);$container.on("click",".f-bind, .f-foreach, .f-on",function(evt){var $target=$(evt.target).is(".f-val")?$(evt.target):$(evt.target).parent().find(".f-val");var variableName=$target.text().trim();var isFunction=$target.parent().is(".f-on");var context=extractor.showContext(variableName,isFunction);$html.find("pre").html(context)}),$container.append($html)}).fail(function(){console.info("Could not get model file contents; this is only available if you're logged in as an admin or team-member.")})};
