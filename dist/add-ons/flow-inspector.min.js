...
"use strict";var ContextExtractor=require("./context-extractor");var utils=require("../../panels/panel-utils.js");module.exports=function($container,configFunction){var template=require("./context-show.html");var $html=$(template);if(!configFunction)return console.warn("You're using an older version of FlowJS which does not support viewing model context."),!1;var config=configFunction();config.model.split(".")[1];var file=new F.service.File(config);file.getContents(config.model,"model").then(function(response){var extractor=new ContextExtractor(config.model,response);$container.on("click",".f-bind, .f-foreach, .f-on",function(evt){var $target=$(evt.target).is(".f-val")?$(evt.target):$(evt.target).parent().find(".f-val");var variableName=$target.text().trim();var isFunction=$target.parent().is(".f-on");var context=extractor.showContext(variableName,isFunction);$html.find("pre").html(context)}),$container.append($html)}).fail(function(){console.info("Could not get model file contents; this is only available if you're logged in as an admin or team-member.")}),utils.makeDraggable($html)};
},{"../../panels/panel-utils.js":10,"./context-extractor":2,"./context-show.html":4}],4:[function(require,module,exports){
...
"use strict";var utils=require("../../panels/panel-utils.js");module.exports=function($container,evtName){var template=require("./legend-panel.html");var $html=$(template);$container.append($html),$container.on("click",":checkbox",function(evt){evt.stopPropagation();var $target=$(evt.target);var type=$target.parent().attr("class").replace("f-","");var subType=$target.parent().children("[class^=f-]").attr("class").replace("f-","");$container.trigger(evtName,type+"-"+subType)}),utils.makeDraggable($html)};
},{"../../panels/panel-utils.js":10,"./legend-panel.html":7}],9:[function(require,module,exports){
"use strict";var panelManager={list:{},register:function(alias,handler){this.list[alias]=handler},getPanel:function(alias){return this.list[alias]}};var availablePanels={context:require("./context-show/context-show-panel"),filter:require("./legend-toggle/legend-panel")};_.each(availablePanels,function(panel,alias){panelManager.register(alias,panel)}),module.exports=panelManager;
},{"./context-show/context-show-panel":3,"./legend-toggle/legend-panel":8}],10:[function(require,module,exports){
"use strict";module.exports=function(){var xOffset=0;var yOffset=0;var isDragged=!1;return{makeDraggable:function($element){$element.on("mousedown",function(evt){return evt.preventDefault(),xOffset=evt.clientX-$element.offset().left,yOffset=evt.clientY-$element.offset().top,isDragged=$element,!1}),$(window.document).on("mousemove",function(evt){return isDragged===$element?(evt.stopPropagation(),$element.css({top:evt.clientY-yOffset,left:evt.clientX-xOffset}),!1):void 0}),$element.on("mouseup",function(evt){isDragged=!1})}}}();
},{}]},{},[1])
