    ...
    describe(':checkbox', function () {
        describe('input handlers', function () {
            it('should trigger the right event on ui change', function () {
                utils.initWithNode('<input type="checkbox" data-f-bind="stuff"/>', domManager).then(function ($node) {
            ...
            describe('On Check', function () {
                it('should pass the right value on check - no default', function () {
                    ...
                    utils.initWithNode('<input type="checkbox" data-f-bind="stuff"/>', domManager, channel).then(function ($node) {
                        var spy = sinon.spy();
                ...
                it('should pass the right value on check - default on', function () {
                    var channel = utils.createDummyChannel();
                    utils.initWithNode('<input type="checkbox" data-f-bind="stuff" value="4"/>', domManager, channel).then(function ($node) {
                        var spy = sinon.spy();
                        $node.on('update.f.ui', spy);
                        $node.prop('checked', true).trigger('change');
            ...
            describe('On UnCheck', function () {
                it('should pass the right value on uncheck - no default', function () {
                    var channel = utils.createDummyChannel();
                    utils.initWithNode('<input type="checkbox" data-f-bind="stuff" checked/>', domManager, channel).then(function ($node) {
                        var spy = sinon.spy();
                        $node.on('update.f.ui', spy);
                ...
                it('should pass the right value on uncheck - default off', function () {
                    var channel = utils.createDummyChannel();
                    utils.initWithNode('<input type="checkbox" data-f-bind="stuff" data-f-off="5" checked/>', domManager, channel).then(function ($node) {
                        var spy = sinon.spy();
                        $node.on('update.f.ui', spy);
        ...
        describe('updaters', function () {
            describe('with default value', function () {
                it('should check itself if the value matches', function () {
                    utils.initWithNode('<input type="checkbox" data-f-bind="stuff" value="3"/>', domManager).then(function ($node) {
                ...
                it('should uncheck itself if the value doesn\'t match', function () {
                    utils.initWithNode('<input type="checkbox" data-f-bind="stuff" value="3" checked/>', domManager).then(function ($node) {
            ...
            describe('without default value', function () {
                ...
                it('should uncheck itself if the value isn\'t truthy', function () {
                    utils.initWithNode('<input type="checkbox" data-f-bind="stuff" checked/>', domManager).then(function ($node) {
                        $node.trigger('update.f.model', { stuff: 0 });
                        var val = $node.prop('checked');
                        val.should.equal(false);
                    });
